...
// The timeline yoyos (plays forward then backward). 
// To make the heart wait 3 seconds total with the message visible, 
// we add 1.5 seconds to the end. (1.5s forward + 1.5s reverse = 3s total)

// Fade in the text quickly when the heart completes (0.3 seconds)
tl.to(messageEl, { opacity: 1, duration: 0.3, ease: "power1.inOut" }, endTime);

// Extend the timeline by 1.5 seconds
tl.set({}, {}, endTime + 1.5);
